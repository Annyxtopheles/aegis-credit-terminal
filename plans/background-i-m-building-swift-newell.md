# B&G Foods ICR Surveillance Dashboard — Comprehensive Product Brief

## Product Identity & Vision

**Product name:** ICR (Investor/Credit Relations) Surveillance Dashboard
**Primary subject company:** B&G Foods, Inc. (NYSE: BGS) — a U.S. packaged food company currently in financial distress with two active covenant violations, an ongoing divestiture of its Green Giant brand assets, and near-term debt maturities beginning September 2027.
...
**Market category:** B2B FinTech / Credit Surveillance SaaS. Positioned between a Bloomberg Terminal (raw data, no curation) and a traditional IR portal (marketing-first, investor-last). ICR fills the gap with curated, editable intelligence at an institutional quality level.
...
- **Brand orange:** `#FF6B35` — ICR logo accent color
- **Alert red:** `#EF4444`
- **Alert amber:** `#F59E0B`
- **Alert green:** `#10B981`
...
- Session persisted in `sessionStorage` under key `'icr-user'`
...
|---|---|---|
| `super_admin` | ICR Admin | Full access to all dashboard pages + Admin Console with all tabs + company switcher + Preview/Dashboard toggle |
...
### Mock credentials
- `admin@icr.com` / `password123` → `super_admin`, 2FA enabled (code `123456`)
- `admin@bgfoods.com` / `password123` → `company_admin` (B&G Foods)
...
1. **Header**: ICR logo (orange "ICR" text + subtitle) + collapse toggle arrow button
2. **ICR Admin only — Mode toggle**: "Dashboard" / "Preview" toggle switch. In Dashboard mode, the super_admin sees the Admin Console nav. In Preview mode, they see the same nav as a normal user (for QA / previewing the client experience)
3. **ICR Admin only — Company selector**: dropdown with search field; 3 companies available: B&G Foods (BGS), Chef Solutions (CHEF), Sprouts Farmers Market (SFM). Selecting a company simulates switching the data context (visual only currently)
...
This is the first thing a user sees on every dashboard page. It communicates urgency and establishes the current operational context for B&G Foods. It is static/hardcoded but is designed to be CMS-editable by ICR Admins.
...
2. `ICRCommentary` (right column) — Amber left-border panel titled "ICR Commentary". Bulleted analysis of largest holder movement data, specifically: Prudential 26.02% of $550M Sr Notes (filed SCH 13G), Capital Advisors 12.08%, BlackRock 9.73%. Notes that "concentrated ownership could create pressure during distress scenarios."
...
**Components:**
1. `ICRCommentary` — Details holder movements: Prudential filed 13G for Sr Unsecured Notes on 2/3/23; Capital Advisors 13G for Sr Secured Notes on 1/27/23; BlackRock on 1/23/23. Commentary warns about concentrated ownership risk in distress scenarios.
...
2. `ICRCommentary` (paired) — Detailed analysis of the divestiture plan: Green Giant brand divestiture announced to reduce leverage; covenant waiver discussions with lender group; restructuring plan execution timeline.

3. Four PNG chart images (imported from PDFs) arranged in a 2×2 grid — these are actual financial charts from B&G Foods financial reports. Charts cover: debt/EBITDA trend, revenue performance, segment breakdown, EBITDA margins.

4. Second `ICRCommentary` below charts — additional analyst notes.
...
2. `EquityPricingChart` — Recharts ComposedChart. BGS equity price (green line, `$4.42`–`$4.95`, 16 data points). Volume bars amber. Dual Y-axes. Ticker watermark `$BGS`.

3. `MarketTrendsPanel` — Cyan left-border panel. 3 bullet points about macro credit market conditions, specifically referencing Fossil Group's debt exchange / UK Restructuring Plan transaction pioneered by Weil Gotshal as a comparable restructuring mechanism. Signals ICR's market intelligence function beyond the subject company.

---

## Admin Console (ICR Admin Only, `/admin`)

The Admin Console is the content management and operations layer for ICR Admins. It is a tab-based interface driven by URL query param `?tab=<name>`.

### Tab: Company Onboarding (`?tab=onboarding`)
Form to onboard a new client company: Company Name, Description, Contact Email. Submit button (visual only). List of existing companies (table stub). Purpose: ICR staff use this to add new companies to the platform.

### Tab: User Management (`?tab=users`)
Full user table with columns: Name, Role, Company, Status (Active/Pending), Last Active, Actions (Edit/Delete). 5 mock users shown. "Invite User" button opens a modal with: pre-generated invite link (copyable), Name, Email, Role selector (User/Company Admin), Company. Purpose: ICR staff manage who has access to which company's dashboard.

### Tab: Commentary Editor (`?tab=commentary`)
Accordion-style CMS for the `ICRCommentary` content that appears on each dashboard page. Three accordion sections:
...
Each section has a textarea (draft mode) and a "Publish" button (visual only). Purpose: ICR analysts write and update the commentary bullets that clients see.
...
Each slot accepts image upload. These correspond to the 4 PNG images shown on the Covenant Compliance page. Purpose: ICR staff upload chart images exported from financial models/PDFs.
...
- Grid of 12 company cards: BGS, CHEF, SFM, PRGO, HZN, ELF, NVST, MATX, MGPI, ARCO, JACK, TDC
...
**Not currently routed** — represents the scale-up vision where ICR tracks 12+ companies simultaneously.
...
2. **Watchlist** — toggle on/off 6 suggested companies: BGS, CHEF, NRTX, SFM, PRGO, HZN
...
## Data Content: B&G Foods Financial Context

All financial data is real (sourced from B&G Foods SEC filings and analyst reports):
...
**Equity:** BGS stock trading ~$4.42–$4.95 (penny-stock territory for a former mid-cap food company, reflecting market's distress pricing)
...
**Divestiture:** Green Giant brand divestiture announced as primary lever to reduce leverage toward covenant compliance
...
ICR is building a two-sided SaaS platform:
- **Supply side:** ICR analysts act as editors — they load company data, write commentary, upload charts, manage clients
- **Demand side:** Institutional credit investors and company IR/finance teams consume the curated intelligence dashboards

The core value proposition: instead of an investor needing to synthesize Bloomberg data + SEC filings + news + 13G filings + covenant documents themselves, ICR pre-digests everything into a single high-density view with analyst commentary layered in.

The B&G Foods dashboard is the first (and currently only) live company implementation, used both as the product and as a sales demonstration for prospective institutional clients.

The unrouted pages (`PortfolioManagerPage`, `CorporateDashboardPage`) show the roadmap: ICR wants to serve both the buy side (portfolio managers tracking many names) and the company itself (IR teams managing investor relationships). The current product is purely buy-side (credit investor monitoring).
...
│   │   ├── ICRCommentary.tsx           # Amber commentary panel
│   │   ├── DebtPricingChart.tsx        # Debt price + volume chart
│   │   ├── EquityPricingChart.tsx      # BGS equity chart
...
│       ├── AdminDashboardPage.tsx      # ICR Admin console (8 tabs)
